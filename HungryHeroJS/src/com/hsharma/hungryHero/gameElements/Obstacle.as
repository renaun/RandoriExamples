	...
	public class Obstacle extends Sprite
	{
		...
		private function createObstacleArt():void
		{
			...
			if (_type == GameConstants.OBSTACLE_TYPE_4)
			{
				...
				{
					obstacleAnimation = new MovieClip(AssetsHungryHero.getAtlas().getTextures("obstacle" + _type + "_0"), 10);
			...
			else
			{
				...
				{
					obstacleImage = new Image(AssetsHungryHero.getAtlas().getTexture("obstacle" + _type));
					this.addChild(obstacleImage);
				}
				else
				{
					// If this object is being reused.
					obstacleImage.texture = AssetsHungryHero.getAtlas().getTexture("obstacle" + _type);
		...
		private function createObstacleCrashArt():void
		{
			...
			{
				// If this is the first time the object is being used.
				obstacleCrashImage = new Image(AssetsHungryHero.getAtlas().getTexture(("obstacle" + _type + "_crash")));
				this.addChild(obstacleCrashImage);
			}
			else
			{
				// If this object is being reused.
				obstacleCrashImage.texture = AssetsHungryHero.getAtlas().getTexture("obstacle" + _type + "_crash");
			}
		...
		private function createLookOutAnimation():void
		{
			...
			{
				// If this is the first time the object is being used.
				lookOutAnimation = new MovieClip(AssetsHungryHero.getAtlas().getTextures("watchOut_"), 10);
